...
def send_to_server():
    ...
    try:
        ...
        #data = rubbish_time + '~' + sound  # ~ to make a split point for the server
        data = rubbish_id + '~' + rubbish_name + '~' + next_sound  # ~ to make a split point for the server
        print(data)
        data = data.encode()
        s.send(data)
...
while soundPlaying == False:
    ...
    if distance <= (initialDistance * 0.5): # less than 50% for tolerance, faster you use the sensor less accurate it is
        ...
        # save uses to file
        usesFile = open('./data/uses.txt', 'w')
        usesFile.write(str(uses))
        usesFile.close()

        # save time of use to file
        usesTime = open('./data/usesTime.txt', 'a')
